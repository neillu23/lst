...
from lst.checkpoint import CheckpointManager, load_from_checkpoint
from lst.config_parser import parse_args_to_pydantic_model
from lst.data.file_util import get_fs
from lst.data.iterators.abstract_iterator import get_state_and_refresh
from lst.data.iterators.multiprocess_iterator import (
    MultiprocessIterator,
    MultiprocessIteratorState,
    PersistType,
)
from lst.data.iterators.packing_iterator import PackingIteratorState
from lst.distributed import (
...
from lst.eval import EVAL_FOLDER_NAME, launch_eval
from lst.logger import init_logger
from lst.metrics import GPUMemoryMonitor, MetricLogger, get_num_params
from lst.model.blt import ByteLatentTransformer
from lst.norms import fixed_clip_grad_norm_
from lst.optim import build_optimizer
from lst.probe import AutoProbeD
from lst.profiling import maybe_run_profiler
from lst.stool import StoolArgs, launch_job
from lst.transformer import (
    LMTransformer,
...
class ExternalStep:
    """shared steps for training curriculum"""
    def __init__(self, ctx: mp.context.BaseContext):
        self._v = ctx.Value('q', 0)  # signed long long
...
def train(args: TrainArgs):
    with ExitStack() as context_stack:
        ...
        ext_step = ExternalStep(ctx)

        data_loader = args.data.build_from_rank(dp_rank, dp_degree, ext_step=ext_step)
        ...
        data_loader = train_state.data_loader_state.build(ext_step=ext_step)
        ...
        while train_state.step < args.steps and (
            ...
            elif args.data.tokenizer_args.name in ["lst"]:
                # TODO: add correct count here
                n_bytes += batch_y.numel() if mask is None else mask.sum()
            else:
                raise ValueError(
