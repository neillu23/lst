...
from lst import ByteLatentError
...
from lst.preprocess.preprocess_entropies import get_id_key, get_text
...
class ArrowFileIterator(StatefulIterator):
    def __init__(
        ...
        *,
        ...
        # Extension detection (e.g., jsonl / on_the_fly / interleaved / hybrid)
        self.extension = None
        ...
        if dataset_files is None:
            ...
            if file_format in ["json", "interleaved", "on_the_fly"]:
                if self.fs is None:
    ...
    def _set_row_num(self, target_row_num: int):
        ...
        logger.info(f"Setting dataset position to {target_row_num} for {data_str}")
        ...
        data_str = maybe_truncate_string(str(self.dataset_files), 200)
        logger.info(
            f"Finished setting dataset position to {target_row_num} for {data_str}"
        )
