...
from lst.data.iterators.sequence_iterator import SequenceIteratorState
...
class SamplingIteratorState(PydanticIteratorState):
    ...
    def build(self, ext_step=None) -> "SamplingIterator":
        return SamplingIterator(
            rng_state=self.rng_state,
            source_to_weight=self.source_to_weight,
            source_to_iterator={
                source: state.build(ext_step=ext_step)
