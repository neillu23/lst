...
from lst.checkpoint import CONSOLIDATE_FOLDER, CheckpointArgs
from lst.data.data_types import Batch
from lst.data.file_util import get_fs
from lst.data.iterators.abstract_iterator import StatefulIterator
from lst.data.iterators.arrow_iterator import ArrowFileIterator
from lst.data.iterators.looping_iterator import LoopingIterator
from lst.data.iterators.multiprocess_iterator import (
    MultiprocessIterator,
    PersistType,
)
from lst.data.iterators.packing_iterator import (
    PackingArgs,
    PackingIterator,
    PackingMode,
)
from lst.data.iterators.preprocess_iterator import PreprocessIterator
from lst.data.iterators.sampling_iterator import SamplingIterator
from lst.data.iterators.sequence_iterator import (
    SequenceIterator,
    SequencePackingArgs,
)
from lst.data.patcher import PatcherArgs, PatchingModeEnum
from lst.distributed import DistributedArgs, EnvironmentArgs
from lst.metrics import LoggingArgs
from lst.model.blt import ByteLatentTransformerArgs
from lst.optim import OptimArgs
from lst.profiling import ProfilerArgs
from lst.tokenizers.build_tokenizer import TokenizerArgs
from lst.transformer import LMTransformerArgs
...
TRAIN_DATA_FILE_PATTERN = ["*.chunk.*.jsonl","*.chunk.*.on_the_fly"]


def _ensure_list_patterns(pat: str | list[str]) -> list[str]:
...
def _union_chunks_by_patterns(
    ...
    """
    Tolerant multi-pattern version:
    - Glob each pattern directly without calling find_and_sanitize_chunks (avoids an assert when any single pattern yields 0).
    - Merge, deduplicate, and sort, then apply the world_size rule once.
    """
    fs = get_fs(dataset_path, s3_profile=s3_profile)
    ...
    all_chunks = []
    for pat in patterns:
        path_with_glob = os.path.join(dataset_path, pat)
        chunks = fs.glob(path_with_glob)  # Returns an empty list if nothing is found; no assert
        for c in chunks:
            if c not in seen:
                seen.add(c)
                all_chunks.append(c)

    # Ensure stable, reproducible rank mapping
    all_chunks.sort()

    n_chunks = len(all_chunks)
    if n_chunks == 0:
        raise ByteLatentError(
            f"No chunks found under dataset_path={dataset_path} with patterns={patterns}"
        )

    if n_chunks > world_size:
        all_chunks = all_chunks[:world_size]
    else:
...
def distribute_data_to_rank(
    *,
    ...
    file_format: str,
    s3_profile: str | None = None,
    file_pattern: str | list[str] = TRAIN_DATA_FILE_PATTERN,
) -> "ArrowFileIterator":
    # Support a single pattern or multiple patterns
    patterns = _ensure_list_patterns(file_pattern)
    dataset_chunks = _union_chunks_by_patterns(dataset_path, world_size, patterns, s3_profile)
    ...
    if rank >= len(rank_to_arrow_iterator_params):
        raise ByteLatentError(
            f"Rank {rank} out of range for total workers {len(rank_to_arrow_iterator_params)}"
        )
...
class DataloaderArgs(BaseModel):
    ...
    def _create_sequence_iterators(
        self, rank: int, world_size: int, ext_step = None
    ...
    def build_from_rank(
        ...
        source_to_sequence_iterators = self._create_sequence_iterators(rank, world_size, ext_step=ext_step)
